...
def scan():
    if request.method == 'POST':
        ...
        if 'http://' and 'https://' not in request.form["domain"]:
            ...
            wayback = request.form.get('wayback')
            getpower = request.form.get('power')
            getregex = request.form.get('reg')
            getwordlist = request.form.get('wordlist')
            if getpower is None:
                power = 5
            else:
            ...
            if getregex is None:
                ...
                regnumber = request.form.get('reg')




            th = Thread(target=task, args=(url,regexselect, wayback, power, regnumber, getwordlist))
                    ...
                    if len(src) > 1:
                        ...
                        for checkdomainlist in src:
                            ...
                            if '/' != str(replacedomain.path[0]):
                                counter1 += 1
                                combinedurls.add("%s/%s" % (url, replacedomain.path))
                            else:
                                counter2 += 1
                                combinedurls.add("%s%s" % (url, replacedomain.path))
